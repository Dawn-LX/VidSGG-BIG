...
class EvalFmtCvtor_v0(object):
    def __init__(self,dataset_type, eval_separately=False):
        ...
        if self.eval_separately:
            # self.part_names = ["top-point","left-edge","right-edge","inner-region"]
            self.part_names = ["top-point","edge","inner-region"]
        else:
    ...
    def to_eval_format_pr(self,proposal,pr_triplet,preserve_ori=False):
    ...
    def prepare_gt(self,gt_graph):
        ...
        start_offset = pred_durations[:,0] - dura_inters[:,0]
        end_offset = pred_durations[:,1] - dura_inters[:,0] + 1
        inter_len = dura_inters[:,1] - dura_inters[:,0] + 1
        
        
        mask_tp = (start_offset == 0) & (end_offset == inter_len)   # Top point of the triangle
        mask_le = (start_offset == 0) & (end_offset < inter_len)    # left edge of the triangle
        mask_re = (start_offset > 0) & (end_offset == inter_len)    # right edge of the traingle
        mask_ed = mask_le | mask_re
        mask_ir = ~(mask_tp | mask_ed)                              # inner regions of the traingle
        assert quintuples.shape[0] == mask_tp.sum() + mask_ed.sum() + mask_ir.sum()
        ...
        parts = {
            ...
            # "right-edge":mask_re, 
            "edge":mask_ed,
            "inner-region":mask_ir
        }
        for name,mask in parts.items():
            gt_infos[name] = (quintuples[mask,:],pred_durations[mask,:])
