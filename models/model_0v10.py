...
class BIG_C(nn.Module): 
    ...
    def __init__(self,config,is_train=True):
        ...
        self.dim_3id  = config.get("dim_i3d",None)
        ...
        if self.dim_3id:
            self.fc_i3d = nn.Sequential(
                nn.Linear(self.dim_3id,self.dim_enti),
                nn.ReLU(),
            )
        ...
        if self.dim_3id:
            self.fc_pred2logits = nn.Linear(self.dim_pred + self.dim_clsme*2 + self.dim_enti*4, self.num_pred_cats)
        else:
            self.fc_pred2logits = nn.Linear(self.dim_pred + self.dim_clsme*2 + self.dim_enti*2, self.num_pred_cats)
    ...
    def encode2decode(self,proposal):
        ...
        traj_i3d = traj_features[:,:,self.dim_feat:]  # shape == (n_trajs, max_frames,dim_i3d)
        if self.dim_3id is None:
            assert traj_visual.shape[2] == self.dim_feat and traj_i3d.shape[2] == 0
        else:
            assert traj_visual.shape[2] == self.dim_feat and traj_i3d.shape[2] == self.dim_3id
        ...
        if self.dim_3id:
            traj_i3d_avg = traj_i3d.mean(dim=1) # shape == (n_enti,dim_i3d)
        else:
            traj_i3d_avg = None
        pred_logits = self.prediction_head(pred_queries,att_matrx,proposal.cat_ids,traj_i3d_avg,enti2enco) # shpae == (n_querys, n_pred_cat)

        return pred_queries,pred_logits,att_matrx
    

    def prediction_head(self,pred_queries,att_matrx,cat_ids,enti_i3d,enti_feat):
        ...
        if self.dim_3id:
            sub_i3d = enti_i3d[pred_soid[0,:],:]  # shape == (n_querys, 832)
            obj_i3d = enti_i3d[pred_soid[1,:],:]  # shape == (n_querys, 832)
            sub_i3d = self.fc_i3d(sub_i3d)
            obj_i3d = self.fc_i3d(obj_i3d)   # (n_querys,dim_enti)
            
            pred_queries = torch.cat([pred_queries,sub_i3d,obj_i3d,sub_feat,obj_feat,sub_clsme,obj_clsme],dim=-1)  # shape == (n_querys,dim_pred+600+4*dim_enti)
        else:
    ...
    def construct_triplet(self,proposal,pred_logits,att_matrx):
        ...
        # print(proposal.video_name,pred2so_ids.tolist(),n_traj)
